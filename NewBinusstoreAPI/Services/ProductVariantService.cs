...
namespace NewBinusstoreAPI.Services
{
    ...
    public class ProductVariantService : BaseService
    {
        public ProductVariantService(ILogger<BaseService> logger) : base(logger)
        {
        }
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                if (Token != "")
                {
                    ...
                    {
                        objJSON.Data = Helper.ProductVariantHelper.Get((ValidTokenModel)processToken.Data);
                    }
                    else
                    {
        ...
        public IActionResult GetSingle(string ID)
        {
            try
            {
                ...
                if (Token != "")
                {
                    ...
                    if (processToken.Status)
                    {
                        objJSON.Data = Helper.ProductVariantHelper.GetSingle((ValidTokenModel)processToken.Data, ID);
        ...
        public IActionResult GetCategoryProduct(string ID)
        {
            try
            {
                ...
                if (Token != "")
                {
                    ...
                    if (processToken.Status)
                    {
                        objJSON.Data = Helper.ProductVariantHelper.GetCategoryProduct((ValidTokenModel)processToken.Data, ID);
                        objJSON.Callback = "onCompleteDetail(e.data)";
                    }
                    else
                    {
        ...
        [Produces("application/json")]
        [ProducesResponseType(typeof(OutputBase), StatusCodes.Status200OK)]
        public IActionResult Save([FromBody] ProductVariantPayload formData)
        {
            try
            {
                var objJSON = new CustomOutput();
                ...
                if (processToken.Status)
                {
                    CustomResponse processData = Helper.ProductVariantHelper.Save((ValidTokenModel)processToken.Data, formData);
