...
namespace NewBinusstoreAPI.Services
{
    [ApiController]
    [Route("BTSPeriod")]
    public class BTSPeriodService : BaseService
    {
        public BTSPeriodService(ILogger<BaseService> logger) : base(logger)
        {
        }
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                if (Token != "")
                {
                    ...
                    {
                        objJSON.Data = Helper.BTSPeriodHelper.Get((ValidTokenModel)processToken.Data);
                    }
                    else
                    {
        ...
        public IActionResult GetSingle(string ID)
        {
            try
            {
                ...
                if (Token != "")
                {
                    ...
                    if (processToken.Status)
                    {
                        objJSON.Data = Helper.BTSPeriodHelper.GetSingle((ValidTokenModel)processToken.Data, ID);
        ...
        [Produces("application/json")]
        [ProducesResponseType(typeof(OutputBase), StatusCodes.Status200OK)]
        public IActionResult Save([FromBody] BTSPeriodPayload formData)
        {
            try
            {
                var objJSON = new CustomOutput();
                ...
                if (processToken.Status)
                {
                    CustomResponse processData = Helper.BTSPeriodHelper.Save((ValidTokenModel)processToken.Data, formData);
