...
namespace NewBinusstoreAPI.Services
{
    ...
    {
        public RoleManagementService(ILogger<BaseService> logger) : base(logger)
        {
        }
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                if (Token != "")
                {
                    ...
                    {
                        objJSON.Data = Helper.RoleManagementHelper.Get((ValidTokenModel)processToken.Data);
                    }
                    else
                    {
        ...
        public IActionResult GetSingle(string ID)
        {
            try
            {
                ...
                if (Token != "")
                {
                    ...
                    if (processToken.Status)
                    {
                        objJSON.Data = Helper.RoleManagementHelper.GetSingle((ValidTokenModel)processToken.Data, ID);
        ...
        [Produces("application/json")]
        [ProducesResponseType(typeof(OutputBase), StatusCodes.Status200OK)]
        public IActionResult Save([FromBody] RoleManagementPayload formData)
        {
            try
            {
                var objJSON = new CustomOutput();
                ...
                if (processToken.Status)
                {
                    CustomResponse processData = Helper.RoleManagementHelper.Save((ValidTokenModel)processToken.Data, formData);
